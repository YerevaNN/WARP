...
class SuperGlueConfig(datasets.BuilderConfig):
    ...
    def __init__(self,
        ...
        """
        # Version history:
        # 1.0.3: Added a few-shot version of SuperGLUE dataset - FewGLUE.
...
def populate_few_shot_configs(builder_configs):
    few_shot_configs = []
    for config in builder_configs:
        if config.few_shot_url is None:
            continue
        ...
        for seed in range(100 + 1):
            ...
            if seed == 0:
                new_config.name = f'few_{new_config.name}'
            else:
                new_config.name = f'few_{new_config.name}_cross_validation_{seed}'
            new_config.is_few_shot = True
            few_shot_configs.append(new_config)

    return builder_configs + few_shot_configs
...
class SuperGlue(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = populate_few_shot_configs([
        SuperGlueConfig(
            name="boolq",
            description=_BOOLQ_DESCRIPTION,
            features=["question", "passage"],
            data_url="https://dl.fbaipublicfiles.com/glue/superglue/data/v2/BoolQ.zip",
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/BoolQ/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/CB/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/COPA/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/MultiRC/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/ReCoRD/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/RTE/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/WiC/",
        ...
        SuperGlueConfig(
            ...
            few_shot_url="https://raw.githubusercontent.com/timoschick/fewglue/master/FewGLUE/WSC/",
    ...
    def _info(self):
        ...
        if dataset_name.startswith('few_'):
            dataset_name = dataset_name[4:]
        if dataset_name.startswith("wsc"):
        ...
        if self.config.pseudolabels_provided:
            features["pseudolabel"] = datasets.features.Sequence(datasets.Value("float"))
    ...
    def _split_generators_for_cross_validation(self, dl_manager):
        config_name, _, seed = self.config.name.partition('_cross_validation_')
        seed = int(seed)
        ...
        if config_name in ["axb", "axg"]:
            raise NotImplementedError

        if not self.config.is_few_shot:
            train_file = os.path.join(dl_dir, "train.jsonl")
        else:
            train_file = os.path.join(self.config.few_shot_url, "train.jsonl")
            train_file = dl_manager.download_and_extract(train_file)
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.train_path is not None:
            train_path = dl_manager.download_and_extract(self.config.train_path)
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.TRAIN,
                    gen_kwargs={
            ...
            train_path = dl_manager.download_and_extract(os.path.join(self.config.few_shot_url, "train.jsonl"))
        else:
        ...
        if self.config.is_few_shot:
            splits.append(datasets.SplitGenerator(
                ...
                gen_kwargs={
                    "data_file": (os.path.join(dl_dir, "unlabeled.jsonl") if not self.config.is_few_shot
                                  else dl_manager.download_and_extract(os.path.join(self.config.few_shot_url, "unlabeled.jsonl"))),
                    "split": datasets.Split("unlabeled"),
                },
            ))

        return splits

    def _generate_examples(self, data_file, split, seed=None):
        generator = self._generate_examples_internal(data_file, split)
        if seed is None:
        ...
        if split == datasets.Split.TRAIN:
            split_indices = indices[:num_examples//2]
        else:
            split_indices = indices[num_examples//2:]
    ...
    def _generate_examples_internal(self, data_file, split):
            ...
            for line in f:
                ...
                if dataset_name.startswith('few_'):
                    ...
                    if isinstance(row["idx"], str):
                        source, _, idx = row["idx"].rpartition("-")
                        known_sources.add(source)
                        if len(known_sources) > 2:
                            raise ValueError("more than one external sources")
                        row["idx"] = 1_000_000_000 + int(idx)
                ...
                if dataset_name == "multirc":
                    ...
                    if "pseudolabel" in row:
                        example["pseudolabel"] = row["pseudolabel"]
                    # else:
                        # example["pseudolabels"] = None
...
class SuperGlueMetrics(datasets.Metric):
    def _info(self):
        supported_configs = [
            "sst2",
            "mnli",
        ...
        return datasets.MetricInfo(
            ...
            features=datasets.Features(
                {
                    "predictions": datasets.Value("int64" if config_name != "stsb" else "float32"),
                    "references": datasets.Value("int64" if config_name != "stsb" else "float32"),
                }
            ),
            codebase_urls=[],
            reference_urls=[],
    ...
    def _compute(self, predictions, references):
        ...
        elif config_name == "stsb":
            return pearson_and_spearman(predictions, references)
        elif config_name in ["mrpc", "qqp"]:
            return acc_and_f1(predictions, references)
        elif config_name in ["cb"]:
